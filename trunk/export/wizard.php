...
<html lang="pl">
    ...
    <body>
        <h1>Generator planów zajęć <img src="../lib/images/betasign.png" alt="" height="12"/></h1>
        <p>
        ...
        ?>
        <?php if ($reg != 0): ?>
            <?php //die('<p class="error">Musisz być w trybie edycji planów zajęć!</p>'); ?>
        <?php endif; ?>
                                ...
                                <td>
                                    ...
                                    <?php else: ?>
                                        <?php for ($i = 1; $i <= $ilgr; $i++): ?>
                                            gr <?php echo $i; ?>
                                            <select name="nl[<?php echo $i; ?>][<?php echo $prz; ?>]" style="width: 90%;">
                                                <option>---</option>
                                                <?php if (count($przed) != 0 || $przsal != 0): ?>
                                                    <?php foreach ($przed as $rowid => $rowcol): ?>
                                                        <option><?php echo $rowcol['nauczyciel']; ?></option>
        ...
        <?php if ($_POST['step'] == '4'): ?>
            <?php unset($_POST); ?>
            <?php header('Location: wizard.php'); ?>
        <?php endif; ?>
        <div id="foot">
            <p>
                <img src="../lib/images/gplv3.png" alt="GNU GPL v3 logo"/>
                <b>Generator Planów Zajęć BETA - <a href="source.php?file=wizard.php" target="_blank">kod źródłowy generatora</a> |
